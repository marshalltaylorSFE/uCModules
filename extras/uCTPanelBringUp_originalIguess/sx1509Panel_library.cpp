						...
						byte tRise, byte tFall)
{
	...
	if (REG_T_RISEARRAY[pin] != 0xFF)
		writeByte(REG_T_RISEARRAY[pin], tRise);
	// Write regTFall
	// 0: off
	// 1-15:  TFall =      (regIOn - (4 * offIntensity)) * tFall * (255/ClkX)
	// 16-31: TFall = 16 * (regIOn - (4 * offIntensity)) * tFall * (255/ClkX)
	if (REG_T_FALLARRAY[pin] != 0xFF)
		writeByte(REG_T_FALLARRAY[pin], tFall);
}
...
void sx1509Class::setAddress( uint8_t inputAddress )
{
